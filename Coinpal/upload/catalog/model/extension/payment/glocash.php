<?php 
class ModelExtensionPaymentGlocash extends Model {
  	public function getMethod($address) {
		...
		if (strlen($class_name) - $index >= 7) {
			$channel = true;
			$pm = substr($class_name, $index + 7);
			$key = "text_title_" . strtolower($pm);
			$title = $this->language->get($key);
			$title = ($title && $title != $key ? $title : (strlen($class_name) - $index==7?'Glocash':$pm.' (Glocash)'));
		}
		...
		$method_data = array();
		$logger->write('payment page c:'.$channel.' s:'.$status);
        $title = $this->config->get('payment_glocash_currtitle');
		if (empty($title)) {
